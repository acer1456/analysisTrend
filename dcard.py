def dcard_run():
  ...
  Original file is located at
      https://colab.research.google.com/drive/10eQDTRTGQ11SRFhuf-bFblzasP6GtXEL
  ...
  stop_words=[]
  f = open(pathaa+'stopwords.txt', 'r', encoding = 'utf8')
  for i in f:
  ...
  #     opts.add_argument('--proxy-server={}'.format(proxy))
      ...
      #偽造header
      opts.add_argument("user-agent={}".format(generate_user_agent()))
      opts.add_argument('Content-Type: application/json')
      driver = webdriver.Chrome(pathaa+'chromedriver', options=opts)
      driver.get(x)
    ...
    #抓文章與留言
  def posts(idd):
      purl = 'https://www.dcard.tw/service/api/v2/posts/' + idd
      data = get(purl)
      s=0
      comments=[]
      # print(data['createdAt'][0:10], data['title'])
      if data['commentCount'] != 0:
        for i in range(1, math.ceil(data['commentCount']/30)+1):
            url = 'https://www.dcard.tw/service/api/v2/posts/' + idd + '/comments?after=' + str(s)
            data_c = get(url)
            if s == 0:
              for b in data_c:
                try:
                  comments.append(b['content'])
                except:
                  continue
            else:
              for b in data_c:
                try:
                  ...
                  continue
            s+=30
      a={'title': data['title'],'content': data['content'], 'forum':data['forumName'], 'time': data['createdAt'][0:10], 'likeCount':data['likeCount'],'comments':comments, 'posturl':purl, 'commentCount':data['commentCount']}
      return a

  def cralwer(url):
      r = requests.get(url, headers = {'Content-Type': 'application/json', 'user-agent': generate_user_agent() })
  ...
  def getforum(forum):
    global df1
    dateright = True
    url = 'https://www.dcard.tw/service/api/v2/forums/'+forum+'/posts?limit=100'
    while dateright == True:
      # print(url)
      data = get(url)
      for i in data:
        timee = i['createdAt'][0:10]
        lastid = str(i['id'])
        if (timeCom(today) > timeCom(timee) >= timeCom(endday)) and timee not in oldlist:
          post = posts(lastid)
          df1 = df1.append(post, ignore_index=True)
          # print(timee, 'Done')
        elif timeCom(today) == timeCom(timee):
          # print(timee, 'Today No')
          pass
        elif timeCom(timee) < timeCom(endday):
          # print(timee, 'other NO')
          dateright=False
      url = 'https://www.dcard.tw/service/api/v2/forums/'+forum+'/posts?limit=100&before=' + lastid
  ...
  if os.path.isfile(pathaa+'dcard.pkl'):
    df_oldlist = pd.read_pickle(pathaa+'dcard.pkl')
    oldlist = list(set(df_oldlist.time))
    if timeCom(sorted(list(set(df_oldlist.time)))[-1]) == timeCom(today)-datetime.timedelta(days=1):
      df1 = df_oldlist
      df_new = df1
      a_comments=0
      ...
      df_new.to_pickle(pathaa+'dcard.pkl')
      usedays=[]
    ...
    for i in list(set(df_new.time)):
      if timeCom(i) >= timeCom(endday):
        usedays.append(i)
    usedays = sorted(usedays)
    df1.to_pickle(pathaa+'dcard.pkl')
  ...
  def combine_comment_content(df1):
    ...
    for i in range(len(df1)):
      for j in range(len(df1['comments'][i])):
        if df1['comments'][i][j] != '':
          a_comments+=1
          b={'content': df1['comments'][i][j], 'time': df1['time'][i]}
  ...
  if os.path.isfile(pathaa+'dcard_cut.pkl'):
    if timeCom(sorted(list(set(df_oldlist.time)))[-1]) == timeCom(today)-datetime.timedelta(days=1):
      df = pd.read_pickle(pathaa+'dcard_cut.pkl')
    else:
      df_cut_old = pd.read_pickle(pathaa+'dcard_cut.pkl')
      df_cut, a_comments = combine_comment_content(df1)
      pcut(df_cut)
      delpsegg(df_cut)
      df = pd.concat([df_cut_old,df_cut], join='inner').reset_index(drop=True)
      df.to_pickle(pathaa+'dcard_cut.pkl')
      df = df[df['time'].isin(usedays)]
  else:
    df_cut, a_comments = combine_comment_content(df1)
    pcut(df_cut)
    delpsegg(df_cut)
    df_cut.to_pickle(pathaa+'dcard_cut.pkl')
    df = pd.read_pickle(pathaa+'dcard_cut.pkl')
  ...
  for i in range(len(df_new)):
      try:
          nrec = int(df_new['likeCount'][i])
          if nrec>=300:
              mostlikepost.append(i)
      except:
          pass
  for i in mostlikepost:
  #     print(dict(df_new.loc[i]))
      df2 = df2.append(dict(df_new.loc[i]),ignore_index=True)
  for i in range(len(df2)):
      df2['title'][i] = '<a href="'+df2['posturl'][i]+'">'+df2['title'][i]+'</a>' +'  '+df2['time'][i]+'  '+str(df2['likeCount'][i])
  df2 = df2[['title']]
  ...
  fig5.update_layout(title='熱門文章(討論度>=300)',width=600, height=500)
  # fig5.show()
  ...
  <h1>Dcard 趨勢分析 <span style="font-size:20px">'''+usedays[0]+'至'+usedays[-1]+'''</span></h1>
  ...
  with open(pathweb+'dcard_'+usedays[0].replace('/','')+'至'+usedays[-1].replace('/','')+'.html', 'w',encoding='utf-8') as f:
      f.write(a)
